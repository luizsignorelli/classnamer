...
random class names by combining randomly-selected elements from arrays that you
provide.

  puts Classnamer.generate([["Foo", "Bar"], ["Baz", "Qux"]]) # => FooQux

You can also specify a custom random number generator. Maybe you don't even
want it to be random.

  puts Classnamer.generate([["Foo", "Bar"], ["Baz", "Qux"]], lambda{|_| 0})
    # => FooBaz

If you plan to use the same customizations multiple times, you can create
a Classnamer::Generator object for that.

  generator = Classnamer::Generator.new([["Foo", "Bar"], ["Baz", "Qux"]])
